...
// The shared form body. Login keeps the single social + email form. Signup is a
// two-step, social-first flow: Step 1 picks a method (Google / Apple / email),
// Step 2 collects the details required for that method.
...
function AuthForm({
  ...
  // ── Login social row (Google signs in; Apple is a stub) ──
  const loginSocial = (
  ...
  const submitBtn = (
    <button type="submit" className={s.submit} disabled={busy || (isSignup && !agreed)}>
      {c.submit}
